...
func enter() -> void:
	...
	GameManager.current_level_name.remove_child(owner)
	GameManager.current_level_name.get_node("OverHUD").add_child(owner)
	# 将非CanvasLayer节点移动到CanvasLayer节点中，需要加上分辨率
	(owner as RigidBody2D).position = gp + Vector2(1920, 1080) * 0.5 * (1 / (GameManager.current_level_name.get_node("Camera2D") as Camera2D).zoom.x)
